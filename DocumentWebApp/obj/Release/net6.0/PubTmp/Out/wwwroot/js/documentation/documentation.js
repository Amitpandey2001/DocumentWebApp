...
const DocumentationManager = {
    ...
    LoadTinyMCE: function () {
        ...
        $('#default').tinymce({
            ...
            plugins: ['code'],
            toolbar: 'code',
            content_style: 'body { font-family:Helvetica,Arial,sans-serif; font-size:14px } pre, code { white-space: pre !important; tab-size: 4; }',
            ...
            // Don't remove redundant <br> tags
            remove_trailing_brs: false,
            // Prevent TinyMCE from removing spaces
            entity_encoding: 'raw',
            // Keep tabs and spaces in code
            keep_styles: true,
            // Prevent automatic paragraph creation
            forced_root_block: false,
            // Setup function to handle code formatting
            setup: function(editor) {
                // Add CSS to preserve whitespace in code blocks on initialization
                editor.on('init', function() {
                    editor.dom.addStyle('pre, code { white-space: pre !important; tab-size: 4; }');
                });
                
                // Handle paste events to preserve indentation
                editor.on('paste', function(e) {
                    // Get the pasted text and preserve its formatting
                    var text = e.clipboardData.getData('text/plain');
                    if (text) {
                        e.preventDefault();
                        // Insert the text as-is, preserving whitespace
